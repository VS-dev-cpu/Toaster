...
void parsePhonemizeConfig(json &configRoot, PhonemizeConfig &phonemizeConfig) {
    ...
    if (configRoot.contains("espeak")) {
        ...
        if (espeakValue.contains("voice")) {
            phonemizeConfig.eSpeak.voice =
                espeakValue["voice"].get<std::string>();
        }
    }
    ...
    if (configRoot.contains("phoneme_map")) {
        ...
        for (auto &fromPhonemeItem : phonemeMapValue.items()) {
            std::string fromPhoneme = fromPhonemeItem.key();
            if (!isSingleCodepoint(fromPhoneme)) {
                // spdlog::error("\"{}\" is not a single codepoint",
                // fromPhoneme);
                throw std::runtime_error(
            ...
            for (auto &toPhonemeValue : fromPhonemeItem.value()) {
                ...
                (*phonemizeConfig.phonemeMap)[fromCodepoint].push_back(
                    toCodepoint);
            }
        }
    }
    ...
    if (configRoot.contains("inference")) {
        ...
        if (inferenceValue.contains("noise_scale")) {
            synthesisConfig.noiseScale =
                inferenceValue.value("noise_scale", 0.667f);
        }

        if (inferenceValue.contains("length_scale")) {
            synthesisConfig.lengthScale =
                inferenceValue.value("length_scale", 1.0f);
        }
...
void synthesize(std::vector<PhonemeId> &phonemeIds,
    ...
    std::vector<int64_t> phomemeIdLengthsShape{
        (int64_t)phonemeIdLengths.size()};
    inputTensors.push_back(Ort::Value::CreateTensor<int64_t>(
    ...
    // From export_onnx.py
    std::array<const char *, 4> inputNames = {"input", "input_lengths",
                                              "scales", "sid"};
    ...
    result.audioSeconds =
        (double)audioCount / (double)synthesisConfig.sampleRate;
    ...
    for (int64_t i = 0; i < audioCount; i++) {
        int16_t intAudioValue = static_cast<int16_t>(std::clamp(
            audio[i] * audioScale,
...
void textToAudio(PiperConfig &config, Voice &voice, std::string text,
    ...
    for (auto phonemesIter = phonemes.begin(); phonemesIter != phonemes.end();
        ...
        if (voice.synthesisConfig.phonemeSilenceSeconds) {
            ...
            auto currentPhrasePhonemes =
                std::make_shared<std::vector<Phoneme>>();
        ...
        // phonemes -> ids -> audio
        for (size_t phraseIdx = 0; phraseIdx < phrasePhonemes.size();
             phraseIdx++) {
